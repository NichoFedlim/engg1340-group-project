...
std::vector<Laser> createXCrossPattern(const BattleBox& box) {
    ...
    int innerMinX = box.getInnerMinX()-1;
    int innerMinY = box.getInnerMinY();
    int innerMaxX = box.getInnerMaxX()+1;
    int innerMaxY = box.getInnerMaxY();
    ...
    while (running) {
        ...
        if (!inAttackCycle && !gameCompleted) {
            ...
            switch (attackRound) {
                ...
                case 4:
                    maxWaves = 3;
                    // If starting a new wave
                    if (waveTimer == 0 && round4State == 0) {
                        // Pick a new safe quadrant that's not the same as last one
                    ...
                    // State 0: Show warning for this wave's safe quadrant
                    if (round4State == 0) {
                        drawDangerQuadrantWarnings(battleBox, round4SafeQuadrant);
                        if (waveTimer >= 120) { // 2 seconds
                            // Begin attack
                    ...
                    // State 1: Attack active
                    else if (round4State == 1) {
                        ...
                        if (allLasersComplete && waveTimer > 60) {
                            round4State = 2;
                            waveTimer = 0;
                            for (auto& laser : lasers) laser.clear();
                            lasers.clear();
                    ...
                    // State 2: Post-attack delay
                    else if (round4State == 2) {
                        if (waveTimer >= 120) {
                            currentWave++;
                            waveTimer = 0;
                            round4State = 0;
                            if (currentWave >= maxWaves) {
                                ...
                                for (auto& laser : lasers) laser.clear();
                                lasers.clear();
                                round4SafeQuadrant = -1; // Reset for next playthrough
                            }
                        }
                    }
                    break;
