...
const int BOMB_SPAWN_INTERVAL = 120;
...
int main() {
    ...
    while (running && !gameOver) {
        ...
        // Separately spawn bombs randomly from the top
        if (frameCount % BOMB_SPAWN_INTERVAL == 0 && bombs.size() < MAX_ENEMIES / 2) { // Spawn bombs more frequently
            ...
            bombs.push_back(std::make_unique<Bomb>(bombX, bombY));
        }
